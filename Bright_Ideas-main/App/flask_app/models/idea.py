...
class Idea:
    ...
    def get_all_ideas(cls):
        query = "SELECT ideas.id as id, ideas.content as content, users.name as sender, users.id AS sender_id, likes.user_id AS liker_id ,count(likes.user_id) as tot_likes FROM ideas JOIN users ON sender_id = users.id LEFT JOIN likes ON idea_id = ideas.id GROUP BY ideas.id ORDER BY tot_likes desc;"
    ...
    @classmethod
    def unlike_idea(cls,data):
        query = "DELETE FROM likes WHERE user_id = %(user_id)s and idea_id = %(idea_id)s;"
        return connectToMySQL(cls.db_name).query_db(query,data)
